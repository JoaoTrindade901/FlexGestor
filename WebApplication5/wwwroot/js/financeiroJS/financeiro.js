...
function popularSelects() {
    ...
    if (selCliente) {
        selCliente.innerHTML = '<option value="">Selecione...</option>' +
            clientes.filter(c => c.fAtivo).map(c => `<option value="${c.idCliente}">${c.nome}</option>`).join("");
    }
    ...
    // Formas e categorias nos modais de pagamento
    ["rp-forma", "pc-forma"].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.innerHTML = formasPagamento.map(f => `<option value="${f.idFormaPagamento}">${f.nome}</option>`).join("");
    });

    const catEntrada = categorias.filter(c => c.Tipo === 1);
    const catSaida = categorias.filter(c => c.Tipo === 2);
    ...
    tbody.innerHTML = pagina.map(c => {
        ...
        return `<tr>
            <td class="area-acoes">
                ...
                <button class="btn-acao btn-calendar-icon" title="Alterar vencimento" onclick="abrirModalVencReceber(${c.idContaReceber}, '${c.dthVencimento?.substring(0, 10)}')">
            ...
            <td><strong>${fmt(restante)}</strong></td>
        </tr>`;
    }).join("");

    document.getElementById("pag-info-receber").textContent =
        `Exibindo ${inicio + 1}–${Math.min(inicio + PAG_RECEBER, total)} de ${total}`;
    renderizarPaginacao("pag-ctrl-receber", pagReceberAtual, Math.ceil(total / PAG_RECEBER), p => { pagReceberAtual = p; renderizarReceber(); });
}
...
function renderizarPagar() {
    ...
    tbody.innerHTML = pagina.map(c => {
        ...
        return `<tr>
            <td class="area-acoes">
                ...
                <button class="btn-acao btn-calendar-icon" title="Alterar vencimento" onclick="abrirModalVencPagar(${c.idContaPagar}, '${c.dthVencimento?.substring(0, 10)}')">
                    <i class="bi bi-calendar-event"></i>
                </button>` : "—"}
            </td>
            <td><span class="status-pill ${getPillClass(status)}">${getPillLabel(status)}</span></td>
    ...
    renderizarPaginacao("pag-ctrl-pagar", pagPagarAtual, Math.ceil(total / PAG_PAGAR), p => { pagPagarAtual = p; renderizarPagar(); });
}
